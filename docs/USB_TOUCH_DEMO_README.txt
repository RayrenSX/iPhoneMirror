...
2. Keep `UsbTouchBridge.exe` beside `UsbTouchDemo.exe`, then run `UsbTouchDemo.exe`.
...
from `dist/UsbTouchBridge.exe` into that folder. The demo is fully self-contained
