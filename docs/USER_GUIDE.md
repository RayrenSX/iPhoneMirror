...
5. 安装完成后从开始菜单启动。顶部语言菜单默认“跟随系统”，也可以切换为简体中文或 English。
...
时回退到 ZIP）、显示百分比和速度，并在 Release 提供 `SHA256SUMS.txt` 时强制校验；随后程序
关闭旧版本、覆盖升级并自动重新启动。下载失败或中断后可以直接重试，残留临时文件会在下次
启动时清理。

顶部“关于”按钮中可手动检查更新，也可设置启动检查、自动下载、正式版提醒和 Beta 版提醒。
跟随系统、深色和浅色主题在主界面的“应用偏好”中设置。
